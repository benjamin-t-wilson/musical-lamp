...
class GithubService {
  ...
  private httpClient = axios.create({ baseURL: this.baseUrl });

  async getPullRequests(username: string, repo: string, state: string = "") {
    const pullRequestBaseUrl: string = `/${username}/${repo}/pulls`;
    const stateModifier: string = state ? `?state=${state}` : "";

    const pullRequestsResponse: AxiosResponse = await this.httpClient.get(
